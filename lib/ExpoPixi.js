...
*/
  ...
  if (!urls) {
    console.error(
      `ExpoPixi.spriteAsync: Cannot parse undefined assets. Please pass valid resources for: ${
        resource
      }.`,
    );
    return;
...
*/
  export function sprite({ localUri: string, width: number, height: number }): PIXI.Sprite {
      if (!localUri || !localUri.toLowerCase().startsWith('file://')) {
        console.error(
      `ExpoPixi.sprite: Please pass valid localUri! ${
        localUri
      } doesn't start with "file://"`,
    );
    return;
      }
    const image = new HTMLImageElement({ localUri, width, height });
